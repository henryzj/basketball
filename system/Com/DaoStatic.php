...
class Com_DaoStatic extends Com_Dao
{
    ...
    public function __call($method, $args)
    {
        ...
        if (method_exists($this, '__CACHE__' . $method)) {
            ...
            // 禁用缓存时
            if (! $this->_isCached) {
                return call_user_func_array(array($this, '__CACHE__' . $method), $args);
            }

            $tblVersion = self::getTblVersion($this->_dbName, $this->_tableName);
            $cacheKey = md5($this->_dbName . ':' . $this->_tableName . ':' . $tblVersion . ':' . $method . ($args ? ':' . serialize($args) : ''));
