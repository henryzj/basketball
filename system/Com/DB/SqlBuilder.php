...
class Com_DB_SqlBuilder
{
    ...
    private function _parseWhere($whereArr)
    {
        ...
        if ($whereArr) {
            if (is_array($whereArr)) {
                $whereSql = 'WHERE ' . $this->__buildWhereSqls($whereArr);
            }
            else {
    ...
    private function __buildWhereSqls(array $whereArr, $_logic = 'AND')
    {
        $whereSqls = [];
        ...
        foreach ($whereArr as $field => $value) {
            ...
            if ($field == '_complex') {
                $__logic = 'AND';
                if (isset($value['_logic']) && $value['_logic']) {
                    $__logic = $value['_logic'];
                    unset($value['_logic']);
                }
                $whereSqls[] = $this->__buildWhereSqls($value, $__logic);
            }
            elseif ($field == '_sql') {
                if (is_array($value)) {
                ...
                $whereSqls[] = $this->__parseWhereOne($field, $value);
            }
        }

        return '(' . implode(' ' . $_logic . ' ',  $whereSqls) . ')';
    }

    // 解析单个WHERE条件
    private function __parseWhereOne($field, $value)
    {
        $whereSql = '';
        $operator = '=';
