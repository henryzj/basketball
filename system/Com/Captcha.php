...
class Com_Captcha
{
     ...
     * 存储本地的 key 前缀
     */
    private $_prefix = '__CaptchaCode_';
    ...
    {
        if ($config) {
            foreach (array('width', 'height', 'space', 'font', 'prefix', 'length', 'storage') as $key) {
    ...
    public function check($word)
    {
        ...
        $given = md5(base64_encode($this->_space . '_' . $word . '_' . $this->_salt));
    ...
    {
        $code = md5(base64_encode($this->_space . '_' . $code . '_' . $this->_salt));
        ...
        if ($this->_storage == 'Cookie') {
            ...
            F('Session')->set($this->_prefix . $this->_space, $code);
        }
    }
    ...
    private function _getStoreCode()
    {
        if ($this->_storage == 'Cookie') {
            ...
            return F('Session')->get($this->_prefix . $this->_space);
        }
    }
    ...
    private function _deleteStoreCode()
    {
        if ($this->_storage == 'Cookie') {
            ...
            F('Session')->del($this->_prefix . $this->_space);
        }
    }

    public function destory()
    {
        return $this->_deleteStoreCode();
    }

    /**
     * 获取存储键名
     */
    public function getStoreKeyName()
    {
        return $this->_prefix . $this->_space;
    }
}
