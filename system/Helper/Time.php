...
class Helper_Time
{
    ...
    public static function getEngTime($time)
    {
        // 获取小时数
        $hour   = floor($time / 3600);
        $min    = floor(($time - $hour * 3600) / 60);
    ...
    {
        $datetime = new DateTime($date, new DateTimeZone(CUR_TIMEZONE));

        return $datetime->format('U');
    }

    // 将时间戳变为真实的时间
    public static function unixtimeToDate($unixtime)
    {
        // DateTime类的bug，加入@可以将Unix时间戳作为参数传入
        $datetime = new DateTime("@$unixtime");
        $datetime->setTimezone(new DateTimeZone(CUR_TIMEZONE));
