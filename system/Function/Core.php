 ...
 * @param int/null $isIncr 自增减
 * @return mixed
 */
function C($field, $value = null, $isIncr = null)
{
    $key = 'Config:' . $field;
    ...
    if (null === $value) {
        ...
        if (! isset($GLOBALS[$key])) {
            $GLOBALS[$key] = F('Redis')->default->hGet('Config', $field);
        }
    ...
    // 写
        ...
        if (null !== $isIncr) {
            $GLOBALS[$key] += $value;
            return F('Redis')->default->hIncrBy('Config', $field, ($isIncr == 1 ? 1 : -1) * $value);
        }

        else {
            $GLOBALS[$key] = $value;
            return F('Redis')->default->hSet('Config', $field, $value);
        }
    }
}
...
function throwx($msg, $code)
{
    throw new Core_Exception_Logic($msg, $code);
}
...
function mUrl($url = '/', array $params = [])
{
    if (strpos($url, 'http://') === false) {
        $url = 'http://' . $GLOBALS['MOBILE_SITE'] . $url;
    }
...
function apiUrl($url = '/', array $params = [])
{
    if (strpos($url, 'http://') === false) {
        $url = 'http://' . $GLOBALS['API_HOST'] . $url;
    }
...
// 对列表进行的字段筛选
function siftListFields(array $list, array $fields)
{
    if (! $list) {
        return [];
    }
    ...
    foreach ($list as &$value) {
        $value = siftDataFields($value, $fields);
    }
...
// 对指定对象进行字段筛选
function siftDataFields(array $orgData, array $fields)
{
    $newData = [];

    foreach ($fields as $key => $field) {
        if (is_array($field)) {
            if (is_array($orgData[$key]) && array_key_exists($key, $orgData)) {
                $newData[$key] = siftDataFields($orgData[$key], $field);
            }
        }
        else {
            if (array_key_exists($field, $orgData)) {
                $newData[$field] = $orgData[$field];
            }
        }
    }
    ...
    foreach ($array as $key => &$value) {
        if (is_array($value)) {
            if (! $value) {
                ...
                $value = setEmptyArrayToNull($value);
            }
        }
    }
