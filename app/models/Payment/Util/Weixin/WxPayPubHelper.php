...
include_once __DIR__ . '/SDKRuntimeException.php';
...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".WX_PARTNER_KEY;
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WX_SSL_CERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WX_SSL_KEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub
{
	...
	public $url;//接口链接
	public $curl_timeout = WX_CURL_TIMEOUT;//curl超时时间
	...
	function createXml()
	{
		...
		$this->parameters["mch_id"] = WX_MCH_ID;//商户号
...
{
	//设置接口链接
	public $url = "https://api.mch.weixin.qq.com/pay/downloadbill";
...
{
	//设置接口链接
	public $url = "https://api.mch.weixin.qq.com/tools/shorturl";
...
class Wxpay_server_pub extends Common_util_pub
{
	...
	function checkSign()
	{
		...
		if (isset($this->data['sign']) && $this->data['sign'] == $sign) {
			return TRUE;
		}
		return FALSE;
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
			...
			$this->returnParameters["mch_id"] = WX_MCH_ID;//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	var $curl_timeout = WX_CURL_TIMEOUT;//curl超时时间
...
/**
* WAP支付——集成微信支付SDK调起支付接口
*/
class Wap_pub extends Common_util_pub
{
	var $parameters;//jsapi参数，格式为json
	var $prepay_id;//使用统一支付接口得到的预支付id
	...
	public function getDeepLink()
	{
		...
		$url = 'weixin：//wap/pay?' . urlencode(http_build_query($params));
